...
import com.ruiteapp.pettranslator.network.XtmObserver
...
object GetHttpDataUtil {
    ...
    fun start() {
        ...
        XtmHttp.toSubscribe(
            ...
            object : XtmObserver<ZZStartRet>() {
                override fun onNext(t: ZZResponseBase<ZZStartRet>) {
                    super.onNext(t)
                    if (t.code == 200) {
...
//                            LZYLog.i("Alex", "okhttp成功  -----toJson=${toJson}")
    ...
    fun getStartRet(): ZZStartRet {
        val gson = Gson()
        val httpRespon = UserInfoModel.getStartHttpRespon();
        if (!TextUtils.isEmpty(httpRespon)) {
            return gson.fromJson(httpRespon, ZZStartRet::class.java)
        } else {
            return gson.fromJson(defaultJson, ZZStartRet::class.java)
        }
        return ZZStartRet()
    }
...
//            .build()
    ...
    fun reportAdReport(
        ...
        XtmHttp.toSubscribe(
            ...
            object : XtmObserver<ZZInstallBean>() {
                override fun onNext(t: ZZResponseBase<ZZInstallBean>) {
                    super.onNext(t)
//                    if(actionType == 0) {
...
//                    }
    ...
    fun setWhiteListHttp(password:String) {
        ...
        XtmHttp.toSubscribe(
            ...
            object : XtmObserver<ZZWhiteListBean>() {
                override fun onNext(t: ZZResponseBase<ZZWhiteListBean>) {
                    super.onNext(t)
                    if (t.code == 200) {
                        if (t.data.status == "0") {
    ...
    fun getCommonConfig(mListener: OnSuccessAndFaultListener) {
        ...
        XtmHttp.toSubscribe(
            ...
            object : XtmObserver<ArrayList<ZZCommonConfigBean>>() {
                override fun onNext(t: ZZResponseBase<ArrayList<ZZCommonConfigBean>>) {
                    super.onNext(t)
                    val gson  = Gson()
                    if (t.code == 200) {
    ...
    fun getIssuesList(mListener: OnSuccessAndFaultListener) {
        ...
        XtmHttp.toSubscribe(
            ...
            object : XtmObserver<ArrayList<ZZHelpQuestionBean>>() {
                override fun onNext(t: ZZResponseBase<ArrayList<ZZHelpQuestionBean>>) {
                    super.onNext(t)
                    val gson  = Gson()
                    if (t.code == 200) {
                        mListener.onSuccess(t.data)
...
//        LZYLog.e("tttt","上传数据："+Gson().toJson(map))
        XtmHttp.toSubscribe(
            ...
            object : XtmObserver<ZZOpenMemberBean>() {
                override fun onNext(t: ZZResponseBase<ZZOpenMemberBean>) {
                    super.onNext(t)
...
//                    LZYLog.e("tttt","上传应用列表返回："+Gson().toJson(t))
    ...
    fun setUnsualIpHttp(type: String) {
        ...
        XtmHttp.toSubscribe(
            ...
            object : XtmObserver<ZZActivateBean>() {
                override fun onNext(t: ZZResponseBase<ZZActivateBean>) {
                    super.onNext(t)
                    if (t.code == 200) {
