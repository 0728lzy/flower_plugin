...
import com.ruiteapp.pettranslator.databinding.ActivityGuideBinding
import com.ruiteapp.pettranslator.ui.fragment.ZZGuideFragment

class ZZGuideActivity : BaseActivity() {

    companion object {
        fun forward(context: Context) {
            context.startActivity(Intent(context, ZZGuideActivity::class.java))
        }
    }
    ...
    private fun forward() {
        if (SPUtils.getInstance().getBoolean("first_permission", true)
            ...
            ZZPermissionActivity.forward(this)
        } else {
    ...
    private inner class ScreenSlidePagerAdapter(activity: FragmentActivity) : FragmentStateAdapter(activity) {
        ...
        override fun createFragment(position: Int): Fragment = ZZGuideFragment(position + 1)
