...
import com.ruiteapp.pettranslator.databinding.ActivityOverviewCallBinding
...
class ZZVideoActivity : BaseActivity() {

    companion object {
        fun show(context: Context, index: Int) {
            context.startActivity(Intent(context, ZZVideoActivity::class.java).apply {
    ...
    private fun goCall() {
        stopTime()
        reset()
        ZZCallActivity.show(this, index)
    }
