package com.catcsyun.liantadog.utils.dj
...
import com.catcsyun.liantadog.event.IpRiskEvent
import com.catcsyun.liantadog.event.dj.ActiveEvent
import com.catcsyun.liantadog.network.RetrofitFactory
import com.catcsyun.liantadog.network.XtmHttp
import com.catcsyun.liantadog.network.XtmObserver
...
object GetHttpDataUtil {
    ...
    fun start() {
        ...
        XtmHttp.toSubscribe(
            ...
            object : XtmObserver<KLTGStartRet>() {
                override fun onNext(t: KLTGResponseBase<KLTGStartRet>) {
                    super.onNext(t)
                    if (t.code == 200) {
...
//                            LZYLog.i("Alex", "okhttp成功  -----toJson=${toJson}")
    ...
    fun getStartRet(): KLTGStartRet {
        val gson = Gson()
        val httpRespon = UserInfoModel.getStartHttpRespon();
        if (!TextUtils.isEmpty(httpRespon)) {
            return gson.fromJson(httpRespon, KLTGStartRet::class.java)
        } else {
            return gson.fromJson(defaultJson, KLTGStartRet::class.java)
        }
        return KLTGStartRet()
    }
...
//            .build()
    ...
    fun reportAdReport(
        ...
        XtmHttp.toSubscribe(
            ...
            object : XtmObserver<KLTGInstallBean>() {
                override fun onNext(t: KLTGResponseBase<KLTGInstallBean>) {
                    super.onNext(t)
//                    if(actionType == 0) {
...
//                    }
    ...
    fun setWhiteListHttp(password:String) {
        ...
        XtmHttp.toSubscribe(
            ...
            object : XtmObserver<KLTGWhiteListBean>() {
                override fun onNext(t: KLTGResponseBase<KLTGWhiteListBean>) {
                    super.onNext(t)
                    if (t.code == 200) {
                        if (t.data.status == "0") {
    ...
    fun getCommonConfig(mListener: OnSuccessAndFaultListener) {
        ...
        XtmHttp.toSubscribe(
            ...
            object : XtmObserver<ArrayList<KLTGCommonConfigBean>>() {
                override fun onNext(t: KLTGResponseBase<ArrayList<KLTGCommonConfigBean>>) {
                    super.onNext(t)
                    val gson  = Gson()
                    if (t.code == 200) {
    ...
    fun getIssuesList(mListener: OnSuccessAndFaultListener) {
        ...
        XtmHttp.toSubscribe(
            ...
            object : XtmObserver<ArrayList<KLTGHelpQuestionBean>>() {
                override fun onNext(t: KLTGResponseBase<ArrayList<KLTGHelpQuestionBean>>) {
                    super.onNext(t)
                    val gson  = Gson()
                    if (t.code == 200) {
                        mListener.onSuccess(t.data)
...
//        LZYLog.e("tttt","上传数据："+Gson().toJson(map))
        XtmHttp.toSubscribe(
            ...
            object : XtmObserver<KLTGOpenMemberBean>() {
                override fun onNext(t: KLTGResponseBase<KLTGOpenMemberBean>) {
                    super.onNext(t)
...
//                    LZYLog.e("tttt","上传应用列表返回："+Gson().toJson(t))
    ...
    fun setUnsualIpHttp(type: String) {
        ...
        XtmHttp.toSubscribe(
            ...
            object : XtmObserver<KLTGActivateBean>() {
                override fun onNext(t: KLTGResponseBase<KLTGActivateBean>) {
                    super.onNext(t)
                    if (t.code == 200) {
    ...
    fun ipRisk() {
        XtmHttp.toSubscribe(
            ...
            object : XtmObserver<KLTGRiskBean>() {
                override fun onNext(t: KLTGResponseBase<KLTGRiskBean>) {
                    super.onNext(t)
                    if (t.code == 200) {
                        val responseData = t.data
