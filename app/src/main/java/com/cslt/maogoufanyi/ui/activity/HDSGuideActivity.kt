...
import com.cslt.maogoufanyi.databinding.ActivityGuideBinding
import com.cslt.maogoufanyi.ui.fragment.HDSGuideFragment

class HDSGuideActivity : BaseActivity() {

    companion object {
        fun forward(context: Context) {
            context.startActivity(Intent(context, HDSGuideActivity::class.java))
        }
    }
    ...
    private fun forward() {
        if (SPUtils.getInstance().getBoolean("first_permission", true)
            ...
            HDSPermissionActivity.forward(this)
        } else {
