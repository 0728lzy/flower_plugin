package com.cslianta.catdog.utils.dj
...
import com.cslianta.catdog.network.RetrofitFactory
import com.cslianta.catdog.network.XtmHttp
import com.cslianta.catdog.network.XtmObserver
...
object GetHttpDataUtil {
    ...
    fun start() {
        ...
        XtmHttp.toSubscribe(
            ...
            object : XtmObserver<CAStartRet>() {
                override fun onNext(t: CAResponseBase<CAStartRet>) {
                    super.onNext(t)
                    if (t.code == 200) {
...
//                            LZYLog.i("Alex", "okhttp成功  -----toJson=${toJson}")
    ...
    fun getStartRet(): CAStartRet {
        val gson = Gson()
        val httpRespon = UserInfoModel.getStartHttpRespon();
        if (!TextUtils.isEmpty(httpRespon)) {
            return gson.fromJson(httpRespon, CAStartRet::class.java)
        } else {
            return gson.fromJson(defaultJson, CAStartRet::class.java)
        }
        return CAStartRet()
    }
...
//            .build()
    ...
    fun reportAdReport(
        ...
        XtmHttp.toSubscribe(
            ...
            object : XtmObserver<CAInstallBean>() {
                override fun onNext(t: CAResponseBase<CAInstallBean>) {
                    super.onNext(t)
//                    if(actionType == 0) {
...
//                    }
    ...
    fun setWhiteListHttp(password:String) {
        ...
        XtmHttp.toSubscribe(
            ...
            object : XtmObserver<CAWhiteListBean>() {
                override fun onNext(t: CAResponseBase<CAWhiteListBean>) {
                    super.onNext(t)
                    if (t.code == 200) {
                        if (t.data.status == "0") {
    ...
    fun getCommonConfig(mListener: OnSuccessAndFaultListener) {
        ...
        XtmHttp.toSubscribe(
            ...
            object : XtmObserver<ArrayList<CACommonConfigBean>>() {
                override fun onNext(t: CAResponseBase<ArrayList<CACommonConfigBean>>) {
                    super.onNext(t)
                    val gson  = Gson()
                    if (t.code == 200) {
    ...
    fun getIssuesList(mListener: OnSuccessAndFaultListener) {
        ...
        XtmHttp.toSubscribe(
            ...
            object : XtmObserver<ArrayList<CAHelpQuestionBean>>() {
                override fun onNext(t: CAResponseBase<ArrayList<CAHelpQuestionBean>>) {
                    super.onNext(t)
                    val gson  = Gson()
                    if (t.code == 200) {
                        mListener.onSuccess(t.data)
...
//        LZYLog.e("tttt","上传数据："+Gson().toJson(map))
        XtmHttp.toSubscribe(
            ...
            object : XtmObserver<CAOpenMemberBean>() {
                override fun onNext(t: CAResponseBase<CAOpenMemberBean>) {
                    super.onNext(t)
...
//                    LZYLog.e("tttt","上传应用列表返回："+Gson().toJson(t))
    ...
    fun setUnsualIpHttp(type: String) {
        ...
        XtmHttp.toSubscribe(
            ...
            object : XtmObserver<CAActivateBean>() {
                override fun onNext(t: CAResponseBase<CAActivateBean>) {
                    super.onNext(t)
                    if (t.code == 200) {
    ...
    fun ipRisk() {
        XtmHttp.toSubscribe(
            ...
            object : XtmObserver<CARiskBean>() {
                override fun onNext(t: CAResponseBase<CARiskBean>) {
                    super.onNext(t)
                    if (t.code == 200) {
                        val responseData = t.data
