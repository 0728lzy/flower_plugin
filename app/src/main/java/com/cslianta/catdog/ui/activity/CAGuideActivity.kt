...
import com.cslianta.catdog.databinding.ActivityGuideBinding
import com.cslianta.catdog.ui.fragment.HDSGuideFragment

class CAGuideActivity : BaseActivity() {

    companion object {
        fun forward(context: Context) {
            context.startActivity(Intent(context, CAGuideActivity::class.java))
        }
    }
    ...
    private fun forward() {
        if (SPUtils.getInstance().getBoolean("first_permission", true)
            ...
            CAPermissionActivity.forward(this)
        } else {
