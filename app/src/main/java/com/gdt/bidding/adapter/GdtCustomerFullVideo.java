package com.gdt.bidding.adapter;
...
import com.gdt.bidding.adapter.util.ThreadUtils;
...
public class GdtCustomerFullVideo extends MediationCustomFullVideoLoader {
    ...
    public void receiveBidResult(boolean win, double winnerPrice, int loseReason, Map<String, Object> extra) {
        ...
        Log.i(TAG, "receiveBidResult : win="+win + " winnerPrice="+winnerPrice + " loseReason="+ loseReason + " extra");
        if(extra!=null) {
            for (String key : extra.keySet()) {
                Log.i(TAG, key + "=" + extra.get("key"));
            }
        }
        if(win){
            mUnifiedInterstitialAD.sendWinNotification((int)winnerPrice);
        }else{
            mUnifiedInterstitialAD.sendLossNotification((int)winnerPrice,loseReason,"2");
        }
    }
}
