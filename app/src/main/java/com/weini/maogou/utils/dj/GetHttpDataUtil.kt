...
import com.weini.maogou.bean.dj.WHCommonConfigBean
import com.google.gson.Gson
import com.weini.maogou.APP
import com.weini.maogou.AppConst
import com.weini.maogou.R
import com.weini.maogou.bean.dj.QCJRiskBean
import com.weini.maogou.bean.dj.WHActivateBean
import com.weini.maogou.bean.dj.WHStartRet
import com.weini.maogou.bean.dj.WHWhiteListBean
import com.weini.maogou.bean.dj.WHHelpQuestionBean
import com.weini.maogou.bean.dj.WHInstallBean
import com.weini.maogou.bean.dj.WHOpenMemberBean
import com.weini.maogou.bean.dj.WHResponseBase
import com.weini.maogou.event.IpRiskEvent
import com.weini.maogou.event.dj.ActiveEvent
import com.weini.maogou.network.RetrofitFactory
import com.weini.maogou.network.XtmHttp
import com.weini.maogou.network.XtmObserver
...
object GetHttpDataUtil {
    ...
    fun start() {
        ...
        XtmHttp.toSubscribe(
            ...
            object : XtmObserver<WHStartRet>() {
                override fun onNext(t: WHResponseBase<WHStartRet>) {
                    super.onNext(t)
                    if (t.code == 200) {
...
//                            LZYLog.i("Alex", "okhttp成功  -----toJson=${toJson}")
    ...
    fun getStartRet(): WHStartRet {
        val gson = Gson()
        val httpRespon = UserInfoModel.getStartHttpRespon();
        if (!TextUtils.isEmpty(httpRespon)) {
            return gson.fromJson(httpRespon, WHStartRet::class.java)
        } else {
            return gson.fromJson(defaultJson, WHStartRet::class.java)
        }
        return WHStartRet()
    }
...
//            .build()
    ...
    fun reportAdReport(
        ...
        XtmHttp.toSubscribe(
            ...
            object : XtmObserver<WHInstallBean>() {
                override fun onNext(t: WHResponseBase<WHInstallBean>) {
                    super.onNext(t)
//                    if(actionType == 0) {
...
//                    }
    ...
    fun setWhiteListHttp(password:String) {
        ...
        XtmHttp.toSubscribe(
            ...
            object : XtmObserver<WHWhiteListBean>() {
                override fun onNext(t: WHResponseBase<WHWhiteListBean>) {
                    super.onNext(t)
                    if (t.code == 200) {
                        if (t.data.status == "0") {
    ...
    fun getCommonConfig(mListener: OnSuccessAndFaultListener) {
        ...
        XtmHttp.toSubscribe(
            ...
            object : XtmObserver<ArrayList<WHCommonConfigBean>>() {
                override fun onNext(t: WHResponseBase<ArrayList<WHCommonConfigBean>>) {
                    super.onNext(t)
                    val gson  = Gson()
                    if (t.code == 200) {
    ...
    fun getIssuesList(mListener: OnSuccessAndFaultListener) {
        ...
        XtmHttp.toSubscribe(
            ...
            object : XtmObserver<ArrayList<WHHelpQuestionBean>>() {
                override fun onNext(t: WHResponseBase<ArrayList<WHHelpQuestionBean>>) {
                    super.onNext(t)
                    val gson  = Gson()
                    if (t.code == 200) {
                        mListener.onSuccess(t.data)
...
//        LZYLog.e("tttt","上传数据："+Gson().toJson(map))
        XtmHttp.toSubscribe(
            ...
            object : XtmObserver<WHOpenMemberBean>() {
                override fun onNext(t: WHResponseBase<WHOpenMemberBean>) {
                    super.onNext(t)
...
//                    LZYLog.e("tttt","上传应用列表返回："+Gson().toJson(t))
    ...
    fun setUnsualIpHttp(type: String) {
        ...
        XtmHttp.toSubscribe(
            ...
            object : XtmObserver<WHActivateBean>() {
                override fun onNext(t: WHResponseBase<WHActivateBean>) {
                    super.onNext(t)
                    if (t.code == 200) {
    ...
    fun ipRisk() {
        XtmHttp.toSubscribe(
            ...
            object : XtmObserver<QCJRiskBean>() {
                override fun onNext(t: WHResponseBase<QCJRiskBean>) {
                    super.onNext(t)
                    if (t.code == 200) {
                        val responseData = t.data
