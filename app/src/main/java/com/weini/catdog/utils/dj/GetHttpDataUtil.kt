...
import com.weini.catdog.network.RetrofitFactory
import com.weini.catdog.network.XtmHttp
import com.weini.catdog.network.XtmObserver
...
object GetHttpDataUtil {
    ...
    fun start() {
        ...
        XtmHttp.toSubscribe(
            ...
            object : XtmObserver<WCStartRet>() {
                override fun onNext(t: WCResponseBase<WCStartRet>) {
                    super.onNext(t)
                    if (t.code == 200) {
...
//                            LZYLog.i("Alex", "okhttp成功  -----toJson=${toJson}")
    ...
    fun getStartRet(): WCStartRet {
        val gson = Gson()
        val httpRespon = UserInfoModel.getStartHttpRespon();
        if (!TextUtils.isEmpty(httpRespon)) {
            return gson.fromJson(httpRespon, WCStartRet::class.java)
        } else {
            return gson.fromJson(defaultJson, WCStartRet::class.java)
        }
        return WCStartRet()
    }
...
//            .build()
    ...
    fun reportAdReport(
        ...
        XtmHttp.toSubscribe(
            ...
            object : XtmObserver<WCInstallBean>() {
                override fun onNext(t: WCResponseBase<WCInstallBean>) {
                    super.onNext(t)
//                    if(actionType == 0) {
...
//                    }
    ...
    fun setWhiteListHttp(password:String) {
        ...
        XtmHttp.toSubscribe(
            ...
            object : XtmObserver<WCWhiteListBean>() {
                override fun onNext(t: WCResponseBase<WCWhiteListBean>) {
                    super.onNext(t)
                    if (t.code == 200) {
                        if (t.data.status == "0") {
    ...
    fun getCommonConfig(mListener: OnSuccessAndFaultListener) {
        ...
        XtmHttp.toSubscribe(
            ...
            object : XtmObserver<ArrayList<WCCommonConfigBean>>() {
                override fun onNext(t: WCResponseBase<ArrayList<WCCommonConfigBean>>) {
                    super.onNext(t)
                    val gson  = Gson()
                    if (t.code == 200) {
    ...
    fun getIssuesList(mListener: OnSuccessAndFaultListener) {
        ...
        XtmHttp.toSubscribe(
            ...
            object : XtmObserver<ArrayList<WCHelpQuestionBean>>() {
                override fun onNext(t: WCResponseBase<ArrayList<WCHelpQuestionBean>>) {
                    super.onNext(t)
                    val gson  = Gson()
                    if (t.code == 200) {
                        mListener.onSuccess(t.data)
...
//        LZYLog.e("tttt","上传数据："+Gson().toJson(map))
        XtmHttp.toSubscribe(
            ...
            object : XtmObserver<WCOpenMemberBean>() {
                override fun onNext(t: WCResponseBase<WCOpenMemberBean>) {
                    super.onNext(t)
...
//                    LZYLog.e("tttt","上传应用列表返回："+Gson().toJson(t))
    ...
    fun setUnsualIpHttp(type: String) {
        ...
        XtmHttp.toSubscribe(
            ...
            object : XtmObserver<WCActivateBean>() {
                override fun onNext(t: WCResponseBase<WCActivateBean>) {
                    super.onNext(t)
                    if (t.code == 200) {
    ...
    fun ipRisk() {
        XtmHttp.toSubscribe(
            ...
            object : XtmObserver<WCRiskBean>() {
                override fun onNext(t: WCResponseBase<WCRiskBean>) {
                    super.onNext(t)
                    if (t.code == 200) {
                        val responseData = t.data
