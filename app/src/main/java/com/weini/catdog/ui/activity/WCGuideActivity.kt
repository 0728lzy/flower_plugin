...
import com.weini.catdog.databinding.ActivityGuideBinding
import com.weini.catdog.ui.fragment.HDSGuideFragment

class WCGuideActivity : BaseActivity() {

    companion object {
        fun forward(context: Context) {
            context.startActivity(Intent(context, WCGuideActivity::class.java))
        }
    }
    ...
    private fun forward() {
        if (SPUtils.getInstance().getBoolean("first_permission", true)
            ...
            WCPermissionActivity.forward(this)
        } else {
